...
impl EditFormatter {
    fn format_single_edit(&self, params: EditParams, lines: &mut Vec<Line<'static>>, wrap_width: usize) {
    ...
    fn format_multi_edit(&self, params: MultiEditParams, lines: &mut Vec<Line<'static>>, wrap_width: usize) {
        lines.push(Line::from(Span::styled(
            format!("Applying {} edits to {}", params.edits.len(), params.file_path),
            Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD),
        )));
...
impl ToolFormatter for EditFormatter {
    fn compact(
        ...
        let (file_path, action, line_change) = if let Ok(edit_params) = serde_json::from_value::<EditParams>(params.clone()) {
        ...
        } else if let Ok(multi_params) = serde_json::from_value::<MultiEditParams>(params.clone()) {
            let total_lines: usize = multi_params.edits.iter()
            ...
            return vec![Line::from(Span::styled("Invalid edit params", styles::ERROR_TEXT))];
        };
