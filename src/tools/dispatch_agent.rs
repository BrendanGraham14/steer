...
impl DispatchAgent {
    ...
    pub async fn execute(&self, prompt: &str) -> Result<String> {
        ...
        let mut api_response = dispatch_client.complete(api_messages.clone(), Some(system_prompt.clone()), Some(tools.clone())).await?;
        ...
        while api_response.has_tool_calls() {
            ...
            for tool_call in &tool_calls {
                ...
                let result = match tool_name.as_str() {
                    "GlobTool" => {
                        ...
                        let path = parameters.get("path").and_then(|p| p.as_str()).unwrap_or(".");
                    ...
                    "GrepTool" => {
                        ...
                        Ok(crate::tools::grep_tool::grep_search(pattern, include, path)?)
                    },
                    "LS" => {
                        let path = parameters["path"].as_str().unwrap_or(".");
                        let ignore = parameters.get("ignore").and_then(|i| i.as_array())
                    ...
                    "View" => {
                        ...
                        let offset = parameters.get("offset").and_then(|o| o.as_u64()).map(|o| o as usize);
                        let limit = parameters.get("limit").and_then(|l| l.as_u64()).map(|l| l as usize);
            ...
            new_api_messages.push(crate::api::Message {
                role: "user".to_string(),
                content_type: crate::api::messages::MessageContent::StructuredContent {
            ...
            api_response = dispatch_client.complete(api_messages.clone(), Some(system_prompt.clone()), Some(tools.clone())).await?;
        }
