...
/// it's used within steer-tools or in an external crate.
///
/// When used in steer-tools, imports will use `crate::`.
/// When used externally, imports will use `steer_tools::`.
#[proc_macro]
...
/// This ensures imports use `steer_tools::` instead of `crate::`.
#[proc_macro]
...
fn tool_impl(input: TokenStream, is_external: bool) -> TokenStream {
    ...
    let (trait_path, context_path, error_path, schema_path, tool_result_path) = if is_external {
        (
            quote! { steer_tools::Tool },
            quote! { steer_tools::ExecutionContext },
            quote! { steer_tools::ToolError },
            quote! { steer_tools::InputSchema },
            quote! { steer_tools::result::ToolResult },
        )
    } else {
        (
            quote! { crate::Tool },
    ...
    let from_impl = if is_external || is_external_result {
        // Skip generating From impls when used outside steer-tools to avoid orphan rules
