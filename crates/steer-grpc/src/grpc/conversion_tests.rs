...
prop_compose! {
    fn arb_mcp_transport()(
        ...
        match variant {
            0 => steer_core::tools::McpTransport::Stdio { command, args },
            1 => steer_core::tools::McpTransport::Tcp { host, port },
            #[cfg(unix)]
            2 => steer_core::tools::McpTransport::Unix { path },
            #[cfg(not(unix))]
            2 => steer_core::tools::McpTransport::Stdio { command, args },
            3 => steer_core::tools::McpTransport::Sse { url: url.clone(), headers: headers.clone() },
            4 => steer_core::tools::McpTransport::Http { url, headers },
            _ => steer_core::tools::McpTransport::Http { url, headers },
        }
    }
}
...
prop_compose! {
    fn arb_operation()(
        variant in 0..2usize,
        cmd in "[a-z ]+",
    ) -> steer_core::app::Operation {
        match variant {
            0 => steer_core::app::Operation::Bash { cmd },
            _ => steer_core::app::Operation::Compact,
        }
    }
}
...
prop_compose! {
    ...
    ) -> steer_core::app::OperationOutcome {
        let elapsed = std::time::Duration::from_millis(elapsed_ms);
        match variant {
            0 => steer_core::app::OperationOutcome::Bash {
                elapsed,
                result: if result_is_ok {
                    Ok(())
                } else {
                    Err(steer_core::app::BashError { exit_code, stderr })
                },
            },
            _ => steer_core::app::OperationOutcome::Compact {
                elapsed,
                result: if result_is_ok {
                    Ok(())
                } else {
                    Err(steer_core::app::CompactError { message: compact_msg })
                },
            },
        }
    }
...
proptest! {
    ...
    fn prop_tool_call_roundtrip(call in arb_tool_call()) {
        let proto = steer_proto::agent::v1::ToolCall {
            id: call.id.clone(),
            ...
            match (original, converted) {
                ...
                (BackendConfig::Mcp { server_name: s1, transport: t1, tool_filter: tf1 },
                 BackendConfig::Mcp { server_name: s2, transport: t2, tool_filter: tf2 }) => {
                    ...
                    match (t1, t2) {
                        (steer_core::tools::McpTransport::Stdio { command: c1, args: a1 },
                         steer_core::tools::McpTransport::Stdio { command: c2, args: a2 }) => {
                            prop_assert_eq!(c1, c2);
                            prop_assert_eq!(a1, a2);
                        }
                        (steer_core::tools::McpTransport::Tcp { host: h1, port: p1 },
                         steer_core::tools::McpTransport::Tcp { host: h2, port: p2 }) => {
                            prop_assert_eq!(h1, h2);
                            prop_assert_eq!(p1, p2);
                        }
                        #[cfg(unix)]
                        (steer_core::tools::McpTransport::Unix { path: p1 },
                         steer_core::tools::McpTransport::Unix { path: p2 }) => {
                            prop_assert_eq!(p1, p2);
                        }
                        (steer_core::tools::McpTransport::Sse { url: u1, headers: h1 },
                         steer_core::tools::McpTransport::Sse { url: u2, headers: h2 }) => {
                            prop_assert_eq!(u1, u2);
                            prop_assert_eq!(h1, h2);
                        }
                        (steer_core::tools::McpTransport::Http { url: u1, headers: h1 },
                         steer_core::tools::McpTransport::Http { url: u2, headers: h2 }) => {
                            prop_assert_eq!(u1, u2);
                            prop_assert_eq!(h1, h2);
                        }
        ...
        match expected {
            ExpectedResult::ProcessUserInput(text) => {
                ...
                if let Ok(Some(client_msg)) = result {
                    ...
                    if let Some(steer_proto::agent::v1::stream_session_request::Message::SendMessage(msg)) = client_msg.message {
            ...
            ExpectedResult::ExecuteCommand(cmd) => {
                ...
                if let Ok(Some(client_msg)) = result {
                    prop_assert_eq!(client_msg.session_id, session_id);
                    if let Some(steer_proto::agent::v1::stream_session_request::Message::ExecuteCommand(msg)) = client_msg.message {
            ...
            ExpectedResult::ExecuteBashCommand(bash_cmd) => {
                ...
                if let Ok(Some(client_msg)) = result {
                    prop_assert_eq!(client_msg.session_id, session_id);
                    if let Some(steer_proto::agent::v1::stream_session_request::Message::ExecuteBashCommand(msg)) = client_msg.message {
            ...
            ExpectedResult::HandleToolResponse { id, approval } => {
                ...
                if let Ok(Some(client_msg)) = result {
                    prop_assert_eq!(client_msg.session_id, session_id);
                    if let Some(steer_proto::agent::v1::stream_session_request::Message::ToolApproval(msg)) = client_msg.message {
                        prop_assert_eq!(msg.tool_call_id, id);
                        match approval {
                            ApprovalType::Denied => {
                                prop_assert_eq!(msg.decision, Some(steer_proto::agent::v1::ApprovalDecision {
                                    decision_type: Some(steer_proto::agent::v1::approval_decision::DecisionType::Deny(true))
                                }));
                            }
                            ApprovalType::Once => {
                                prop_assert_eq!(msg.decision, Some(steer_proto::agent::v1::ApprovalDecision {
                                    decision_type: Some(steer_proto::agent::v1::approval_decision::DecisionType::Once(true))
                                }));
                            }
                            ApprovalType::AlwaysTool => {
                                prop_assert_eq!(msg.decision, Some(steer_proto::agent::v1::ApprovalDecision {
                                    decision_type: Some(steer_proto::agent::v1::approval_decision::DecisionType::AlwaysTool(true))
                                }));
                            }
                            ApprovalType::AlwaysBashPattern(pattern) => {
                                prop_assert_eq!(msg.decision, Some(steer_proto::agent::v1::ApprovalDecision {
                                    decision_type: Some(steer_proto::agent::v1::approval_decision::DecisionType::AlwaysBashPattern(pattern.clone()))
                                }));
                            }
                        }
                    } else {
    ...
    fn prop_started_event_roundtrip(op in arb_operation()) {
        ...
        if let AppEvent::Started { id: roundtrip_id, op: roundtrip_op } = roundtrip {
            ...
            match (&op, &roundtrip_op) {
                (steer_core::app::Operation::Bash { cmd: c1 },
                 steer_core::app::Operation::Bash { cmd: c2 }) => {
                    prop_assert_eq!(c1, c2);
                }
    ...
    fn prop_finished_event_roundtrip(outcome in arb_operation_outcome()) {
        ...
        if let AppEvent::Finished { id: roundtrip_id, outcome: roundtrip_outcome } = roundtrip {
            ...
            match (&outcome, &roundtrip_outcome) {
                (steer_core::app::OperationOutcome::Bash { elapsed: e1, result: r1 },
                 steer_core::app::OperationOutcome::Bash { elapsed: e2, result: r2 }) => {
                ...
                (steer_core::app::OperationOutcome::Compact { elapsed: e1, result: r1 },
                 steer_core::app::OperationOutcome::Compact { elapsed: e2, result: r2 }) => {
                    prop_assert_eq!(e1, e2);
                    match (r1, r2) {
                        (Ok(()), Ok(())) => {},
                        (Err(err1), Err(err2)) => {
