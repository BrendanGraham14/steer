...
impl InputPanelState {
    ...
    pub async fn handle_fuzzy_key(
        ...
        // Ensure the cursor is still in a valid fuzzy-query position. If the
        // user moved the cursor outside the query (e.g. with ← / → / Home /
        // End or deleted back past the trigger) we close the fuzzy-finder so it
        // no longer intercepts keys.
        if !self.is_in_fuzzy_query() {
        ...
        if self.fuzzy_finder.mode() == FuzzyFinderMode::Files {
            ...
            if let Some(query) = self.get_current_fuzzy_query() {
                ...
                // Empty query – keep fuzzy finder open but clear results
                self.fuzzy_finder.update_results(Vec::new());
                None
            }
        } else {
            None
