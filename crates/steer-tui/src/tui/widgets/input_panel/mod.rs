...
pub use fuzzy_state::FuzzyFinderState;
...
impl InputPanelState {
    ...
    pub fn get_current_fuzzy_query(&self) -> Option<String> {
        if self.is_in_fuzzy_query() {
            ...
            FuzzyFinderState::get_current_fuzzy_query(
                trigger_pos,
                self.get_cursor_byte_offset(),
                &self.content(),
            )
    ...
    pub fn complete_fuzzy_finder(&mut self, selected_path: &str) {
        if let Some(trigger_pos) = self.fuzzy_finder.trigger_position() {
            ...
            let (new_content, new_row, new_col) = FuzzyFinderState::complete_fuzzy_finder(
                &content,
                trigger_pos,
                cursor_offset,
                selected_path,
    ...
    pub fn complete_command_fuzzy(&mut self, selected_command: &str) {
        if let Some(trigger_pos) = self.fuzzy_finder.trigger_position() {
            ...
            let (new_content, new_row, new_col) = FuzzyFinderState::complete_command_fuzzy(
                &content,
                trigger_pos,
                cursor_offset,
                selected_command,
