...
mod tests {
    ...
    async fn test_grep_relative_path_glob_matching() {
        ...
        // Create nested directory structure similar to steer project
        fs::create_dir_all(temp_dir.path().join("steer/src/session")).unwrap();
        fs::create_dir_all(temp_dir.path().join("steer/src/utils")).unwrap();
        fs::create_dir_all(temp_dir.path().join("other/src")).unwrap();

        // Create test files
        fs::write(
            temp_dir.path().join("steer/src/session/state.rs"),
            "pub struct SessionConfig {\n    pub field: String,\n}",
        )
        .unwrap();
        fs::write(
            temp_dir.path().join("steer/src/utils/session.rs"),
        ...
        let params = GrepParams {
            ...
            include: Some("steer/src/**/*.rs".to_string()),
            path: None,
        };
        let params_json = serde_json::to_value(params).unwrap();
        ...
        // Should find 3 matches in steer/src files but not in other/src
        assert_eq!(result.0.matches.len(), 3);
        assert!(result.0.matches.iter().any(|m| {
            m.file_path.contains("steer/src/session/state.rs")
                && m.line_number == 1
                && m.line_content == "pub struct SessionConfig {"
        }));
        assert!(result.0.matches.iter().any(|m| {
            m.file_path.contains("steer/src/utils/session.rs")
                && m.line_number == 2
